...
// V.IMP = we have to connect it with our server.and start acessing the clients requests through it:so for doing that we need to use the (listen) method of our server:after that we are ready to start our server:for this particular port number:and start listening for requests:which are coming on this port number.
...
// V.IMP = the (listen) method. it takes two arguments:the port number and the callback function:the callback function basically tells us that our server is up and running by showing some text or console-statement which we have given to him :it is bascially listening our Listener:
