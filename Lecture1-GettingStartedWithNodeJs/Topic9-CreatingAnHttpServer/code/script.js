...
// -> through which we are gonna be able to initiate the communication-protocol on our server application:
...
const server = http.createServer((req, res) => {
  // response has (end) method in it:which basically stop the request response cycle of the particular request:and sends response back to the client:acc to its request:
  res.end("hey! every one welcome to my first NodeJs Server");
});

// => 3 = third after creating server. we have to define the (port) for our server.through which basically we are gonna be able to provide the unique identity to our server and also make it accessible to our clients:
...
// V.IMP = the (listen) method. it takes two arguments:the port number and the callback function:the callback function basically tells us that our server is up and running by showing some text on console-statement which we have given to him :this callback function is basically listening our Listener:
