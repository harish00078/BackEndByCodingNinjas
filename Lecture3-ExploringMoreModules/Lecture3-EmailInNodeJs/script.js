...
// -> V.IMP: for configuring the (email) and its (emailer-service) server through which we are gonna be able to send-emails to the (userS):
// we gonna do that with the help of (transporter) or (transporter-object):which is basically gonna help us to send the emails to the (email-server):and then that (email-server) is gonna be sending the email to the (user):
