...
// [Second-way]: Here we also use the (import) keyword to import things:but if we only want to import the particular thing from the exported-file:then we have to use the (curly-brackets) or (object) instead of (*) and (as) keywords:and with in those curly-brackets or (object) we basically import our things:
