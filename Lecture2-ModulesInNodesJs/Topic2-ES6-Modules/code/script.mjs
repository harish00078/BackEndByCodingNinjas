...
// V.V.IMP(NOTE) = so when we are using the (ES6) module-syntax:To (import) and (export) Modules from one file to another file:when we are mostly creating the application simply on the (nodejs) environment without the help of any framework or library:
//then we have to use the (mjs) extension in the file-name:instead of simple (js) extension.
