...
//(IMP-NOTE) = here we can see that:so when we are basically importing the same file again:then it did not gonna work because of nodejs's (module-caching) mechanism: because when we import our files using (commonjs) and (ES6) syntax-methods.then in there first time imports these files basically get stored with in the (cache) mechanism of nodejs:so that's why when we are importing it again it will not work:because system gonna be still have that same with in him or in its (caching) mechanism:
